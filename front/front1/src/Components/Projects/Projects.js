...
class Projects extends Component {
  ...
  next() {
    ...
    fetch(n + "&name=" + this.state.search1 + "&date=" + this.state.search2, {
      headers: {
        Accept: "application/json",
        Authorization: "Bearer " + token,
      },
  ...
  handleEditModal(e) {
    if (e != undefined) {
      ...
      fetch("http://localhost:8000/api/project/" + e, {
        ...
        .then((res) => res.text())
        .then((res) => this.setState({ projectEdited: JSON.parse(res) }));
      this.setState({ editName: this.state.projectEdited.name });
    } else this.setState({ projectEdited: "" });
  ...
  handleEdit = async (e) => {
    ...
    try {
      ...
      // var date = d.toString();
      await Axios.post(
        "http://localhost:8000/api/project/" + edit,
        {
  ...
  render() {
      ...
      for (let i = 0; i < this.state.projects.data.length; i++) {
        ...
        array.push(
          <div className={"row" + c + " rowData"}>
            <div className="col0 col2">{this.state.projects.data[i].name}</div>
            <div className="col0 col2">{this.state.projects.data[i].date}</div>
            <div className="col0 col2">
              <div className="action">
                <div className="manage-project" title="Manage Project">
                  <div
                    className="manage"
                    onClick={() => {
                      this.handleManageModal(this.state.projects.data[i].id);
                    }}
                  >
                    <FontAwesomeIcon className="task pointer" icon={faEdit} />
                  </div>
                </div>
                <div
                  itle="Edit"
                  onClick={() => {
                    this.handleEditModal(this.state.projects.data[i].id);
                  }}
                >
                ...
                <div title="Delete">
                  <FontAwesomeIcon
                    onClick={() => {
                      this.deleteProject(this.state.projects.data[i].id);
                    }}
    ...
    console.log(this.state.projectEdited);
    return (
      <div className="App">
        ...
        <div className="table project-tbl">
          ...
          <div>
            <div className="row1">
              <div className="col0 col2">Project Name</div>
