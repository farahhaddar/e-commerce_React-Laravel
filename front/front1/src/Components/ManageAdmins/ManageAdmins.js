...
class ManageAdmins extends Component {
  ...
  search() {
    fetch(
      ...
      .then((res) => {
        ...
        } else this.setState({ admins: JSON.parse(res) });
      });
  }
  last() {
  ...
  onSubmit(e) {
    ...
    console.log(this.state.image);
    if (id == 0)
      fetch("http://localhost:8000/api/admin/admin", {
        method: "post",
        body: formData,
        headers: {
          Accept: "application/json",
        ...
        .then((res) => {
          if (res.status == "Not authorized") {
            ...
            res &&
              res.error &&
              toast.error(
                res.error.message[Object.keys(res.error.message)][0],
                {
                  position: "top-right",
                  autoClose: 5000,
                  hideProgressBar: true,
  ...
  deleteAdmin(e) {
    fetch("http://localhost:8000/api/admin/admin/" + e, {
      method: "delete",
      headers: {
        Accept: "application/json",
        Authorization: "Bearer " + token,
  ...
  editAdmin(e) {
    ...
    fetch("http://localhost:8000/api/admin/admin/" + e, {
      ...
      .then((res) => {
        ...
        this.setState({ name: this.state.adminEdited.data.name });
        this.setState({ email: this.state.adminEdited.data.email });
  ...
  render() {
    ...
    if (this.state.admins.data != undefined) {
      count = this.state.admins.data.total;
      var last_page = this.state.admins.data.prev_page_url;
      var next_page = this.state.admins.data.next_page_url;
      var nbPages = this.state.admins.data.last_page;
    ...
    if (this.state.admins.data != undefined) {
      console.log(this.state.admins.data.data.length);

      for (let i = 0; i < this.state.admins.data.data.length; i++) {
        let c = i % 2;
        array.push(
            ...
            <div className="col0 col2">
              <img
                src={
                  ...
                  this.state.admins.data.data[i].image
                }
                height="50px"
                width="50px"
                style={{ borderRadius: "50%" }}
              />
              {this.state.admins.data.data[i].name}
            </div>
            <div className="col0 col2">
              {this.state.admins.data.data[i].email}
            </div>
            <div className="col0 col2">
              <div className="action">
                <div title="Edit">
                  <FontAwesomeIcon
                    color={" #e55d87"}
                    onClick={() => {
                      this.editAdmin(this.state.admins.data.data[i].id);
                    }}
                    className="pen"
                    title="Edit"
                    icon={faPen}
                  ...
                  <FontAwesomeIcon
                    onClick={() => {
                      if (
                        ...
                        this.deleteAdmin(this.state.admins.data.data[i].id);
                    }}
                    className="trash"
                    icon={faTrash}
                  />
          ...
          <Modal.Body>
            <label className="label">
              <input
                ...
                className="text"
                placeholder=" "
                defaultValue={this.state.adminEdited.data.name}
                onChange={this.handleInputChange}
              />
              <span className="input-type"> Name </span>
            </label>
            <label className="label">
              <input
                ...
                className="text"
                placeholder=" "
                defaultValue={this.state.adminEdited.data.email}
                onChange={this.handleInputChange}
              />
              <span className="input-type"> Email Address </span>
            </label>
