...
class Orders extends Component {
  ...
  deleteOrder(e) {
    fetch("http://localhost:8000/api/admin/Order/" + e, {
      method: "delete",
      // body: formData,
      headers: {
  ...
  search() {
    fetch(
      ...
      .then((res) => res.text())
      .then((res) => this.setState({ orders: JSON.parse(res).data }));
  }
  next() {
  ...
  choice = (id, choice) => {
    //alert(id); //id of the order
    //alert(choice); //value
    let formData = new FormData();
    formData.append("status", choice);
    formData.append("_method", "PUT");
    fetch("http://localhost:8000/api/admin/updateStatus/" + id, {
      method: "post",
      body: formData,
      headers: {
        Accept: "application/json",
  ...
  render() {
    ...
    let count = "";
    if (this.state.orders.data != undefined) {
      count = this.state.orders.total;
      var last_page = this.state.orders.prev_page_url;
      var next_page = this.state.orders.next_page_url;
      var nbPages = this.state.orders.last_page;
    ...
    var array = [];
    if (this.state.orders.data != undefined) {
      for (let i = 0; i < this.state.orders.data.length; i++) {
        let c = i % 2;
        {
          /* ttt */
        }
        let state = [];
        state.push(<option value="1">Waiting</option>);
        state.push(<option value="2">Delivered</option>);
        ...
        for (let j = 0; j < state.length; j++) {
          choose.push(
            <Dropdown.Item
              onClick={() =>
                this.choice(this.state.orders.data[i].id, state[j].props.value)
              }
              // href="#/action-1"
            >
              {state[j].props.children}
            </Dropdown.Item>
          );
        }
        array.push(
          <div className={"row" + c + " rowData"}>
            <div className="col0 col4">{this.state.orders.data[i].address}</div>
            <div className="col0 col4">{this.state.orders.data[i].comment}</div>
            <div className="col0 col4">{this.state.orders.data[i].price}</div>
            <div className="col00 col4">
              {this.state.orders.data[i].status == 1 || this.state.orders.data[i].status == undefined  ? (
                <Dropdown>
                  <Dropdown.Toggle
                    variant="success"
                    className={"dropDown1"}
            ...
            <div className="col0 col4">
              <div className="action">
                <div title="Products">
                  <Link
                    ...
                    to={{
                      ...
                      state: {
                        orderId: this.state.orders.data[i].id,
                      },
                    }}
                  >
                    {" "}
                ...
                <div
                  itle="Edit"
                  onClick={() => {
                    // this.handleEditModal(this.state.orders.data[i].id);
                    this.userInfo(this.state.orders.data[i].user_id);
                  }}
                >
                  <FontAwesomeIcon
                    className="pen"
                ...
                {/* <div title="Delete">
                  <FontAwesomeIcon
                    onClick={() => {
                      this.deleteOrder(this.state.orders.data[i].id);
                    }}
                    className="trash"
                    icon={faTrash}
                  />
    ...
    return (
      <div className="App">
        ...
        <div className="table project-tbl">
          ...
          <div>
            <div className="row1">
              <div className="col0 col4">Address</div>
              <div className="col0 col4">Comment</div>
