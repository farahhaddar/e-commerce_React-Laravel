...
class WorkoutCategories extends Component {
  constructor(props) {
    super(props);
    this.state = {
      role: "",
  ...
  search() {
    fetch(
      "http://localhost:8000/api/workoutCategories/" +
        this.state.rows +
        "?page=1&name=" +
        this.state.search1,
      {
      ...
      .then((res) => res.text())
      .then((res) => this.setState({ workoutCategories: JSON.parse(res) }));
  }
  last() {
  ...
  handleModal() {
    this.setState({ workoutCategoryEdited: "" });
    this.setState({ image: "" });
    id = 0;
  ...
  componentDidMount() {
    this.props.title("Workout Categories");
    let a = document.getElementsByClassName("pageOptions");
    a[0].style.display = "none";
  ...
  onSubmit(e) {
    ...
    console.log(this.state.image);
    if (id == 0)
      fetch("http://localhost:8000/api/admin/workoutCategories", {
        method: "post",
        body: formData,
        headers: {
          Accept: "application/json",
      ...
      fetch("http://localhost:8000/api/admin/workoutCategories/" + id, {
        method: "post",
        body: formData,
        headers: {
          Accept: "application/json",
  ...
  deleteWorkoutCategory(e) {
    fetch("http://localhost:8000/api/admin/workoutCategories/" + e, {
      method: "delete",
      headers: {
        Accept: "application/json",
        Authorization: "Bearer " + token,
  ...
  editWorkoutCategory(e) {
    id = e;
    fetch("http://localhost:8000/api/workoutCategory/" + e, {
      headers: {
        // Accept: "application/json",
        // Authorization: "Bearer " + token,
      },
      ...
      .then((res) => res.text())
      .then((res) => {
        this.setState({ workoutCategoryEdited: JSON.parse(res) });
        this.setState({ name: this.state.workoutCategoryEdited.data.name });
        // this.setState({ image: this.state.workoutCategoryEdited.data.image });
      });
    if (this.state.show == true) {
      flag = 0;
      flagNav = 0;
  ...
  render() {
    console.log(this.state.workoutCategories);
    let count = "";
    if (this.state.workoutCategories.data != undefined) {
      count = this.state.workoutCategories.data.total;
      var last_page = this.state.workoutCategories.data.prev_page_url;
      var next_page = this.state.workoutCategories.data.next_page_url;
      var nbPages = this.state.workoutCategories.data.last_page;
    ...
    if (this.state.workoutCategories.data != undefined) {
      console.log(this.state.workoutCategories.data.data.length);

      for (let i = 0; i < this.state.workoutCategories.data.data.length; i++) {
        let c = i % 2;
        array.push(
            ...
            <div className="col0 col1">
              <img
                src={
                  ...
                  this.state.workoutCategories.data.data[i].image
                }
                height="50px"
                width="50px"
                style={{ borderRadius: "50%" }}
              />
              {this.state.workoutCategories.data.data[i].name}
            </div>
            <div className="col0 col1">
              <div className="action">
                <div title="Edit">
                  <FontAwesomeIcon
                    color={" #e55d87"}
                    onClick={() => {
                      this.editWorkoutCategory(
                        this.state.workoutCategories.data.data[i].id
                      );
                    }}
                    className="pen"
                    title="Edit"
    ...
    let form = [];
    if (this.state.workoutCategoryEdited == "")
      form.push(
          ...
          <Modal.Body>
            <label className="label">
              <input
                ...
                className="text"
                placeholder=" "
                defaultValue={this.state.workoutCategoryEdited.data.name}
                onChange={this.handleInputChange}
              />
              <span className="input-type"> Name </span>
            </label>
    ...
    return (
      <div className="App">
        ...
        <div className="table">
          ...
          <div>
            <div className="row1">
              <div className="col0 col1">Name</div>
